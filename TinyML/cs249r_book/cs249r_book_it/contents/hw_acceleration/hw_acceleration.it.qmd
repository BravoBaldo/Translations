...
L'apprendimento automatico è emerso come una tecnologia trasformativa in molti settori. Tuttavia, l'implementazione delle capacità di ML nei dispositivi edge del mondo reale affronta problemi dovuti alle risorse di elaborazione limitate. L'accelerazione hardware specializzata è essenziale per abilitare l'apprendimento automatico ad alte prestazioni in base a questi vincoli. Gli acceleratori hardware ottimizzano le operazioni ad alta intensità di elaborazione come l'inferenza utilizzando silicio personalizzato ottimizzato per le moltiplicazioni di matrici. Ciò fornisce accelerazioni drastiche rispetto alle CPU per uso generico, sbloccando l'esecuzione in tempo reale di modelli avanzati su dispositivi con limitazioni di dimensioni, peso e potenza.

Questo capitolo fornisce informazioni di base essenziali sulle tecniche di accelerazione hardware per l'apprendimento automatico embedded e sui relativi compromessi. L'obiettivo è fornire ai lettori gli strumenti per effettuare selezioni hardware e ottimizzazioni software consapevoli per sviluppare funzionalità di apprendimento automatico performanti sui dispositivi.
...
+-------------------------------------------------------+----------------+--------+
| Inviaew 1 KB byte su una rete da 1 Gbps               | 10.000 ns      | 10 us  |
+-------------------------------------------------------+----------------+--------+
...
+-------------------------------------------------------+----------------+--------+
| Inviare pacchetto CA → Paesi Bassi → CA               | 150.000.000 ns | 150 ms |
+-------------------------------------------------------+----------------+--------+
...
Anche [Intel Arc Graphics](https://www.intel.com/content/www/us/en/products/details/fpga/stratix/10/nx.html) e [AMD Radeon RX](https://www.amd.com/en/graphics/radeon-rx-graphics) hanno sviluppato le loro GPU nel tempo.
...
Ad esempio, l'ultima GPU [H100](https://www.nvidia.com/en-us/data-center/h100/) di Nvidia fornisce 4000 TFLOP di FP8, 2000 TFLOP di FP16, 1000 TFLOP di TF32, 67 TFLOP di FP32 e 34 TFLOP di FP64 come prestazioni di elaborazione, che possono accelerare notevolmente l'addestramento di grandi batch su modelli come BERT, GPT-3 e altre architetture di trasformatori. Il parallelismo scalabile delle GPU è fondamentale per accelerare il deep learning computazionalmente intensivo.

##### Ecosistema Software Maturo

Nvidia fornisce ampie librerie di runtime come [cuDNN](https://developer.nvidia.com/cudnn) e [cuBLAS](https://developer.nvidia.com/cublas) che sono altamente ottimizzate per primitive di deep learning. Framework come TensorFlow e PyTorch si integrano con queste librerie per abilitare l'accelerazione GPU senza programmazione diretta. CUDA fornisce un controllo di livello inferiore per calcoli personalizzati.
...
#### Vincoli della Distribuzione Incorporata
...
* **MLPerf:** Include un ampio set di benchmark che coprono sia l'addestramento [@mattson2020mlperf] che l'inferenza [@reddi2020mlperf] per una gamma di attività di machine learning.
