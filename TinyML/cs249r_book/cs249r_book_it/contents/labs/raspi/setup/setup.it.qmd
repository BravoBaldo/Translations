...
> Le istruzioni generali per *Rasp-5* si applicano anche alle versioni più vecchie di Raspberry Pi, come Rasp-3 e Raspi-4.
...
> Negli esempi qui, useremo nomi host diversi: raspi, raspi-5, raspi-Zero, ecc. Lo si deve sostituire con quello in uso.
...
Il modo più semplice per interagire con Rasp-Zero è tramite SSH ("Headless"). Si può usare un terminale (MAC/Linux), [PuTTy (](https://www.putty.org/)Windows) o qualsiasi altro.
...
Quindi, attivare di nuovo lo swapfile e riavviare Rasp-zero:
...
- Utilizza la configurazione headless (SSH) per risparmiare risorse.
...
- Può eseguire senza problemi l'intero ambiente desktop.
- Per Pi 5, valutare l'utilizzo di un dissipatore attivo per la gestione della temperatura durante attività intensive.

Ricordarsi di adattare i requisiti del progetto in base allo specifico modello di Raspberry Pi in uso. Il Pi Zero è ideale per progetti a basso consumo energetico e con limiti di spazio, mentre i modelli Pi 4/5 sono più adatti per attività che richiedono un utilizzo intensivo di risorse di calcolo.
