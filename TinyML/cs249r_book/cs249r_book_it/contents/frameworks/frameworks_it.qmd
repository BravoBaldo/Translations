...
* I layers vengono utilizzati indirettamente durante la creazione di modelli. I tensori scorrono tra i livelli durante l'esecuzione.
...
Di recente, la distinzione si è offuscata poiché i framework adottano entrambe le modalità. TensorFlow 2.0 passa automaticamente alla modalità di grafo dinamico, consentendo agli utenti di lavorare con quelli statici quando necessario. La dichiarazione dinamica offre flessibilità e facilità d'uso, rendendo i framework più intuitivi, mentre i grafi statici forniscono vantaggi di ottimizzazione a scapito dell'interattività. Il framework ideale bilancia questi approcci. Ecco una tabella che confronta i pro e i contro dei grafi di esecuzione statici e dinamici:

| Esecuzione Grafo | Pro | Contro |
| --- | --- | --- |
| Statico (Declare-then-execute) | Abilita le ottimizzazioni del grafo visualizzando il modello completo in anticipo<br/>Può esportare e distribuire grafi congelati<br/>Il grafico è impacchettato indipendentemente dal codice | Meno flessibile per la ricerca e l'iterazione<br/>Le modifiche richiedono la ricostruzione del grafico<br/>L'esecuzione ha fasi di compilazione ed esecuzione separate |
| Dinamico (Define-by-run) | Stile imperativo intuitivo come il codice Python<br/>Interlaccia la creazione del grafo con l'esecuzione<br/>Grafi facili da modificare<br/>Il debug si adatta perfettamente al flusso di lavoro | Più difficile da ottimizzare senza un grafo completo<br/>Possibili rallentamenti dovuti alla creazione di grafi durante l'esecuzione<br/>Potrebbe richiedere più memoria |
...
Senza benchmark unificati, non esisteva un modo standard per valutare e confrontare le capacità di diverse piattaforme hardware di fornitori come NVIDIA, Arm e Ambiq Micro.. Le valutazioni esistenti si basavano su benchmark proprietari pensati per mostrare i punti di forza di specifici chip. Ciò rendeva impossibile misurare i miglioramenti hardware in modo oggettivo, imparziale e imparziale. Il capitolo [Benchmarking AI](../benchmarking/benchmarking. cmd) tratta questo argomento in modo più dettagliato.
